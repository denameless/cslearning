...
#define MAX_PAGE_QUEUE_SIZE 1024

struct PageQueue {
    int pages[MAX_PAGE_QUEUE_SIZE];
    int front;
    int rear;
    int size;
    ...
    bool isFull() {
        return size == MAX_PAGE_QUEUE_SIZE;
    }
    ...
    bool enqueue(int page) {
        if (isFull()) return false;
        pages[rear] = page;
        rear = (rear + 1) % MAX_PAGE_QUEUE_SIZE;
        size++;
        return true;
    }
    ...
    int dequeue() {
        ...
        front = (front + 1) % MAX_PAGE_QUEUE_SIZE;
        size--;
        return result;
    }
};
