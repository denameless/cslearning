...
printf("Error");
    ...
    for (int i = 0; i < count; ++i, vaddress += PAGE_SIZE)
    {
        ...
        if (!physicalPageAddress) {
            int oldVaddr = fifoQueue.dequeue();
            if (oldVaddr != -1) {
                int oldPaddr = vaddr2paddr(oldVaddr);

                // 失效旧页表项
                int *oldPte = (int *)toPTE(oldVaddr);
                *oldPte = 0;
		releaseVirtualPages(type, oldVaddr, 1);
                physicalPageAddress = oldPaddr;
            } else {
                flag = false;
            }
        }
